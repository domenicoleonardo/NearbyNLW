package com.rocketseat.nlw.nearby.data.model

import com.rocketseat.nlw.nearby.data.model.Rule

val mockRules = listOf(
    Rule(
        id = "012576ea-4441-4b8a-89e5-d5f32104c7c4",
        descriptio = "Disponível até 31/12/2024",
        marketId = "012576ea-4441-4b8a-89e5-d5f32104c7c4"
    ),
    Rule(
        id = "312576ea-4341-1b8a-83e5-d5f32104c7b1",
        descriptio = "Válido apenas para consumo no local",
        marketId = "bde73364-95c5-46e4-8804-79a7ca3824c4"
    )
)
